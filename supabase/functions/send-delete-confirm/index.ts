...
const EMAIL_FROM = Deno.env.get('EMAIL_FROM') || 'noreply@mail.runbickers.com';

const ALLOWED_ORIGINS = (Deno.env.get('CORS_ALLOWED_ORIGINS') || 'https://jeongwooshin.github.io,https://btcmobick.runbickers.com')
...
function json(req: Request, status: number, body: unknown) {
  return new Response(JSON.stringify(body), {
    status,
    headers: { 'Content-Type': 'application/json', ...makeCorsHeaders(req.headers.get('origin')) },
  });
}

Deno.serve(async (req) => {
  if (req.method === 'OPTIONS') return new Response('ok', { headers: makeCorsHeaders(req.headers.get('origin')) });
  ...
  try {
    ...
    try {
      ...
      if (!emailRes.ok) {
        ...
        return json(req, 502, { error: '이메일 전송에 실패했습니다.', resend_status: emailRes.status, resend_error: text.slice(0, 1000) });
      }
    } catch (e) {
